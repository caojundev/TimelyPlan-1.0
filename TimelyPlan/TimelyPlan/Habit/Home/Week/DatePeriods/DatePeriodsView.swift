...
protocol DatePeriodsViewDelegate: AnyObject {
    
    /// 获取统计周期数组
    func periodsInDatePeriodsView(_ view: DatePeriodsView) -> [DatePeriod]?
    
    /// 时间段对应单元格类
    func datePeriodsView(_ view: DatePeriodsView,
                         cellClassForPeriod period: DatePeriod) -> AnyClass
    
    /// 单元格出队列
    func datePeriodsView(_ view: DatePeriodsView,
                         didDequeCell cell: UICollectionViewCell,
                         forPeriod period: DatePeriod)
    
    /// 选中一个时间段
    func datePeriodsView(_ view: DatePeriodsView,
                         didSelectPeriod period: DatePeriod)
    
    /// 点击是否高亮时间段对应的单元格
    func datePeriodsView(_ view: DatePeriodsView,
                         shouldHighlightPeriod period: DatePeriod) -> Bool
}

class DatePeriodsView: TPCollectionWrapperView,
                       TPCollectionSingleSectionListDataSource,
                       TPCollectionViewAdapterDataSource,
                       TPCollectionViewAdapterDelegate {

    /// 代理对象
    weak var delegate: DatePeriodsViewDelegate?
     
    /// 单元格最小宽度
    var minCellWidth = 40.0
    
    /// 时间段数组
    private(set) var periods: [DatePeriod] = []
    ...
    /// 重新加载与特定时间段相交的所有单元格
    func reloadPeriods(intersects otherPeriod: DatePeriod) {
        var periodsToUpdate = [DatePeriod]()
    ...
    func cells(intersect period: DatePeriod) -> [UICollectionViewCell] {
    ...
    func cellForPeriod(_ period: DatePeriod) -> UICollectionViewCell? {
    ...
    func adapter(_ adapter: TPCollectionViewAdapter, classForCellAt indexPath: IndexPath) -> AnyClass? {
        let period = adapter.item(at: indexPath) as! DatePeriod
